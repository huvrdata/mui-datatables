...
> **Note:** This is a maintained fork of [gregnb/mui-datatables](https://github.com/gregnb/mui-datatables). For version history and documentation prior to v5, see the [original project](https://github.com/gregnb/mui-datatables).
...
# Table of contents
- [MUI-Datatables - Datatables for MUI](#mui-datatables---datatables-for-mui)
...
| mui-datatables | MUI            | Required Dependencies                               |
|----------------|----------------|------------------------------------------------------|
| ^5.0.0         | ^5.0.0, ^6.0.0 | `@mui/material`, `@mui/icons-material`, `@emotion/react`, `@emotion/styled` |

For compatibility with older versions of Material-UI (v3/v4), see the [original project](https://github.com/gregnb/mui-datatables).

## Demo

Browse live examples and demos at **[mui-datatables.vercel.app](https://mui-datatables.vercel.app)**.
...
|**`confirmFilters`**|boolean|false|Works in conjunction with the **customFilterDialogFooter** option and makes it so filters have to be confirmed before being applied to the table. When this option is true, the customFilterDialogFooter callback will receive an applyFilters function which, when called, will apply the filters to the table. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/serverside-filters/index.js)
...
|**`customFooter`**|function||Render a custom table footer. `function(count, page, rowsPerPage, changeRowsPerPage, changePage, `[`textLabels: object`](https://github.com/huvrdata/mui-datatables/blob/master/src/textLabels.js)`) => string`&#124;` React Component` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-footer/index.js)
...
|**`customSort`**|function||Override default sorting with custom function. If you just need to override the sorting for a particular column, see the sortCompare method in the [column options](#column-options). `function(data: array, colIndex: number, order: string) => array` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-sorting/index.js)
|**`customTableBodyFooterRender`**|function||Render a footer under the table body but above the table's standard footer. This is useful for creating footers for individual columns. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-footer/index.js)
...
|**`enableNestedDataAccess`**|string|""|If provided a non-empty string (ex: "."), it will use that value in the column's names to access nested data. For example, given a enableNestedDataAccess value of "." and a column name of "phone.cell", the column would use the value found in `phone:{cell:"555-5555"}`. Any amount of nesting will work. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/data-as-objects/index.js) demonstrates the functionality.
|**`expandableRows`**|boolean|false|Enable/disable expandable rows. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/expandable-rows/index.js)
...
|**`filter`**|boolean or string|true|Show/hide filter icon from toolbar. Possible values:<p><ul><li>true: Button is visible and clickable.</li><li>false: Button is not visible.</li><li>disabled: Button is visible, but not clickable.</li></ul></p>
...
|**`fixedHeader`**|boolean|true|Enable/disable a fixed header for the table [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/fixed-header/index.js)
|**`fixedSelectColumn`**|boolean|true|Enable/disable fixed select column. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/fixed-header/index.js)
...
|**`jumpToPage`**|boolean|false|When true, this option adds a dropdown to the table's footer that allows a user to navigate to a specific page. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/large-data-set/index.js)
...
|**`onFilterChipClose`**|function||Callback function that is triggered when a user clicks the "X" on a filter chip. `function(index : number, removedFilter : string, filterList : array) => void` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/serverside-filters/index.js)
|**`onFilterConfirm`**|function||Callback function that is triggered when a user presses the "confirm" button on the filter popover. This occurs only if you've set **confirmFilters** option to true. `function(filterList: array) => void` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/serverside-filters/index.js)
...
|**`print`**|boolean or string|true|Show/hide print  icon from toolbar. Possible values:<p><ul><li>true: Button is visible and clickable.</li><li>false: Button is not visible.</li><li>disabled: Button is visible, but not clickable.</li></ul></p>
|**`renderExpandableRow`**|function||Render expandable row. `function(rowData, rowMeta) => React Component` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/expandable-rows/index.js)
|**`resizableColumns`**|boolean|false|Enable/disable resizable columns.
|**`responsive`**|string|'vertical'|Enable/disable responsive table views. Options: <p><ul><li>"vertical" (default value): In smaller views the table cells will collapse such that the heading is to the left of the cell value.</li><li>"standard": Table will stay in the standard mode but make small changes to better fit the allocated space.<li>"simple": On very small devices the table rows will collapse into simple display.</li></ul></p>[Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/simple/index.js)
...
|**`search`**|boolean or string|true|Show/hide search icon from toolbar. Possible values:<p><ul><li>true: Button is visible and clickable.</li><li>false: Button is not visible.</li><li>disabled: Button is visible, but not clickable.</li></ul></p>
|**`searchPlaceholder`**|string||Search text placeholder. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-search/index.js)
|**`searchProps`**|object|{}|Props applied to the search text box. You can set method callbacks like onBlur, onKeyUp, etc, this way. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-search/index.js)
...
|**`setFilterChipProps`**|function||Is called for each filter chip and allows you to place custom props on a filter chip. `function(colIndex: number, colName: string, filterValue: string) => object` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-filter/index.js)
|**`setRowProps`**|function||Is called for each row and allows you to return custom props for this row based on its data. `function(row: array, dataIndex: number, rowIndex: number) => object` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-styling/index.js)
|**`setTableProps`**|function||Is called for the table and allows you to return custom props for the table based on its data. `function() => object` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-styling/index.js)
|**`sort`**|boolean|true|Enable/disable sort on all columns.
|**`sortFilterList`**|boolean|true|Enable/disable alphanumeric sorting of filter lists.
|**`sortOrder`**|object|{}|Sets the column to sort by and its sort direction. To remove/reset sorting, input in an empty object. The object options are the column name and the direction: `name: string, direction: enum('asc', 'desc')` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-columns/index.js)
...
|**`viewColumns`**|boolean or string|true|Show/hide viewColumns icon from toolbar. Possible values:<p><ul><li>true: Button is visible and clickable.</li><li>false: Button is not visible.</li><li>disabled: Button is visible, but not clickable.</li></ul></p>
...
|**`customFilterListOptions`**|object|| (These options only affect the filter chips that display after filters are selected. To modify the filters themselves, see `filterOptions`) <p><ul><li>`render`: function that returns a string or array of strings used as the chip label(s). `function(value) => string OR arrayOfStrings` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-filter/index.js)</li><li>`update`: function that returns a `filterList (see above)` allowing for custom filter updates when removing the filter chip. filterType must be set to "custom". `function(filterList, filterPos, index) => filterList` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-filter/index.js)</li></ul></p>
...
|**`filterList`**|array||Filter value list [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/column-filters/index.js)
|**`filterOptions`**|object||<p><i>These options affect the filter display and functionality from the filter dialog. To modify the filter chips that display after selecting filters, see `customFilterListOptions`</i></p><p>This option is an object of several options for customizing the filter display and how filtering works.</p><p><ul><li>names: custom names for the filter fields [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/column-filters/index.js)</li><li>logic: custom filter logic [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-filter/index.js)</li><li>display(filterList, onChange(filterList, index, column), index, column, filterData): Custom rendering inside the filter dialog [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-filter/index.js). `filterList` must be of the same type in the main column options, that is an array of arrays, where each array corresponds to the filter list for a given column.</li><li>renderValue: A function to customize filter choices [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-filter/index.js). Example use case: changing empty strings to "(empty)" in a dropdown.</li><li>fullWidth (boolean): Will force a filter option to take up the grid's full width.</li></ul></p>
...
|**`setCellHeaderProps`**|function||Is called for each header cell and allows you to return custom props for the header cell based on its data. `function(columnMeta: object) => object` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-styling/index.js)
|**`setCellProps`**|function||Is called for each cell and allows to you return custom props for this cell based on its data. `function(cellValue: string, rowIndex: number, columnIndex: number) => object` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-styling/index.js)
|**`sort`**|boolean|true|Enable/disable sorting on column.
|**`sortCompare`**|function||Custom sort function for the column. Takes in an order string and returns a function that compares the two column values. If this method and options.customSort are both defined, this method will take precedence. `(order) => ({data: val1}, {data: val2}) => number` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/column-sort/index.js)
|**`sortDescFirst`**|boolean|false|Causes the first click on a column to sort by desc rather than asc. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-columns/index.js)
|**`sortThirdClickReset`**|boolean|false|Allows for a third click on a column header to undo any sorting on the column. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-columns/index.js)
...
|Name|Type|Default|Description
|:--:|:-----|:--|:-----|
|**`debounceSearchRender`**|function||Function that returns a function for the customSearchRender method. This plug-in allows you to create a debounced search which can be useful for server-side tables and tables with large data sets. `function(debounceWait) => function` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/large-data-set/index.js)

## Customize Styling

Using MUI theme overrides will allow you to customize styling to your liking. First, determine which component you would want to target and then lookup the override classname. Let's start with a simple example where we will change the background color of a body cell to be red:
 ...
 * `Checkbox` - A special 'data-description' prop lets you differentiate checkboxes [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/custom-components/index.js). Valid values: ['row-select', 'row-select-header', 'table-filter', 'table-view-col'].The dataIndex is also passed via the "data-index" prop.
 * `ExpandButton` [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/expandable-rows/index.js)
 * `DragDropBackend`
 * `TableBody`
...
* `icons` - An object containing optional replacement icon classes for the actions
  toolbar. [Example](https://github.com/huvrdata/mui-datatables/blob/master/examples/customize-toolbar-icons/index.js)
	* `SearchIcon`
	* `DownloadIcon`
	* `PrintIcon`
	* `ViewColumnIcon`
...
For more information, please see this [example](https://github.com/huvrdata/mui-datatables/blob/master/examples/custom-components/index.js). All examples can be viewed live at [mui-datatables.vercel.app](https://mui-datatables.vercel.app).
...
To see an example **[Click Here](https://github.com/huvrdata/mui-datatables/blob/master/examples/serverside-pagination/index.js)**
